...
void j1Map::Draw()
{
	...
	while (item_layer != NULL)
	{
		...
		for (uint row = 0; row<data.height; row++)
		{
			...
			{
				LOG("%i");
				if (layer->data[layer->Get(column, row)] != 0)
				{
					iPoint mapPoint = MapToWorld(column, row);
					SDL_Rect section = item_tileset->data->GetTileRect(layer->data[layer->Get(column, row)]);
...
iPoint j1Map::WorldToMap(int x, int y) const
{
	...
	else if (data.type == MapTypes::MAPTYPE_ISOMETRIC)
	{
		...
		ret.y = ((y * 2) / data.tile_height) -((x / data.tile_width) + (y / data.tile_height));
	}
	return ret;
}
